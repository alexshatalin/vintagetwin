<?php require_once(__DIR__ . '/vt_header.php'); ?>
<section class="secondary_page_container favorites">
	<section class="favorites__hero">
		<h1>Favorites</h1>
		<p>Click the <i class="fas fa-heart"></i> to bookmark your item<br /> to shop later.</p>	
	</section>
	
	<section class="favorites__search_results">
		<div class="container">
			<div class="row">
				<div class="content">
					<div class="results_number">450 results</div>
					<?php require_once(__DIR__ . '/components/category_filter.php'); ?>

					<div class="favorites__body grid">
						<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 grid_item color_black">
							<a href="#">
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>
						<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 grid_item color_blue original_prints">
							<a href="#"> 
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>
						<div class="col-xs-6 col-sm-4  col-md-3 col-lg-3 grid_item color_black brands_logos original_prints">
							<a href="#">
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>
						<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 grid_item color_orange brands_logos colleges">
							<a href="#">
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>

						<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 grid_item color_green colleges">
							<a href="#">
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>
						<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 grid_item color_blue stripes_patterns">
							<a href="#">
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>
						<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 grid_item color_white">
							<a href="#">
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>
						<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 grid_item color_blue">
							<a href="#">
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>

						<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 grid_item color_black">
							<a href="#">
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>
						<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 grid_item color_orange">
							<a href="#">
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>
						<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 grid_item color_white">
							<a href="#">
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>
						<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 grid_item color_black">
							<a href="#">
								<picture>
									<div class="favorite_button"><i class="far fa-heart"></i></div>
								  	<img src="images/na_slide1.jpg" alt="Featured Product" />
								</picture>
								<div class="slide_content">
									<div class="slide_category">UNISEX - Lookin For Action</div>
									<div class="product_type">Barely There Tee </div>
									<div class="price">$ 48</div>
								</div>
							</a>
						</div>

					</div>
					<? require_once(__DIR__ . '/components/category_pagination.php'); ?>
				</div>
			</div>
		</div>
	</section>
</section>


<?php require_once(__DIR__ . '/vt_footer.php'); ?>	